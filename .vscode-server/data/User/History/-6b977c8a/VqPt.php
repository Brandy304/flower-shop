...
// 花卉管理的路由组，只有花店员工(floreist)能够访问
Route::middleware(['auth', 'role:florist'])->prefix('flowers')->name('flowers.')->group(function () {
    Route::get('/', [FlowerController::class, 'index'])->name('index');  // 列出所有花卉，渲染 index 视图
    Route::get('/create', [FlowerController::class, 'create'])->name('create');  // 创建新花卉
    Route::post('/', [FlowerController::class, 'store'])->name('store');  // 保存新花卉
...
Route::get('/florist-dashboard', [FlowerController::class, 'index'])->name('flowers.index');
// 其他身份验证相关的路由
